...
from doctor_spider.items.doctor_item import DoctorItem
...
class DoctorSpider(CrawlSpider):
    ...
    # 开始爬取的url
    start_urls = ["http://www.mingyihui.net/doctors/"]

    # 从页面需要提取的url 链接(link)
    links = LinkExtractor(allow=["http://www.mingyihui.net/doctors/pn_\d+/",
                                 # "http://www.mingyihui.net/\d+_doctors/",
                                 # "http://www.mingyihui.net/\d+_doctors/pn\d+/",
    ...
    custom_settings = {
        "ITEM_PIPELINES": {
            'doctor_spider.pipelines.doctor_pipelines.DoctorPipeline': 300,
        }
    }
    ...
    def parse_content(self, response):
        ...
        for element in response.xpath('//ul[@class="H_main"]/li[@class="H_list"]'):
            ...
            try:
                #/html/body/div[8]/div[1]/div[4]/div/ul/li[1]/div[1]/h3/a
