...
class DBConnection(object):
    def __init__(self, dbs_read, dbs_write=None):
        # logging.debug("will create db instance, callstack:%s", detailtrace())
        ...
        try:  # 读连接
            ...
            self.conn_read = None
            logging.error("connect db conn_read(%s) Error %d: %s", str(self.cf_read), ex.args[0], ex.args[1])
        ...
        try:  # 同步写连接
            ...
            logging.error("connect db conn_write(%s) Error %d: %s", str(self.cf_write), ex.args[0], ex.args[1])
    ...
    def reconnect_db(self, _type):
        ...
        if _type == constant.CONN_READ:
            try:
                ...
                logging.error("reconnect db conn_read(%s) Error %d: %s", str(self.cf_read), e.args[0], e.args[1])
                self.conn_read = None
            ...
            try:
                ...
                logging.error("reconnect db conn_write(%s) Error %d: %s", str(self.cf_write), e.args[0], e.args[1])
                self.conn_write = None
    ...
    def is_connect_living(self, _type):
        ...
        try:
            ...
            logging.error(ex, exc_info=1)
            logging.error("is_connect_living judge failed. _type:%s" % _type)
            return False
    ...
    def fetchone(self, sql, *args):
        ...
        try:
            ...
            if not self.is_connect_living(constant.CONN_READ):
                logging.error("[db_source_read:%s] connect gone away, try to reconnect.", self.db_source_read)
            ...
            logging.error("[db_source_read:%s] query sql(%s) error:%s", self.db_source_read, str(sql), str(e))
            raise e
        finally:
            if cursor:
                cursor.close()
    ...
    def execute_sql(self, sql):
        ...
        try:
            if not self.is_connect_living(constant.CONN_WRITE):
                logging.error("[db_source_write:%s] connect gone away, try to reconnect", self.db_source_write)
