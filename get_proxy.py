...
class Proxy(object):
    ...
    def getContent(self, num):
        ...
        for index, i in enumerate(result_even):
            t1 = i.xpath("./td/text()")[:2]
            print "IP:%s\tPort:%s" % (t1[0], t1[1])
            if self.isAlive(t1[0], t1[1]):
                print self.now, t1[0], t1[1]
                ok_proxy_list.append("http://%s:%s" % (t1[0], t1[1]))
        for index, i in enumerate(result_odd):
